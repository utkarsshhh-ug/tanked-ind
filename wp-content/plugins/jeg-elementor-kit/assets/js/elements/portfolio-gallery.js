! function(e) {
    ...
    class r extends elementorModules.frontend.handlers.Base {
        ...
        bindEvents() {
            this.onRenderInit(), this.onClickHover()
        }
        onRenderInit() {
            const e = this.elements.$row_items,
                t = this.elements.$image_items;
            jQuery(e.get().reverse()).each(function() {
                jQuery(this).hasClass("current-item") && (e.removeClass("current-item"), jQuery(this).addClass("current-item"))
            }), jQuery(t.get().reverse()).each(function() {
                jQuery(this).hasClass("current-item") && (t.removeClass("current-item"), jQuery(this).addClass("current-item"))
            })
        }
        onClickHover() {
            const e = this,
            ...
            t.hasClass("on-click") && r.each(function() {
                jQuery(this).on({
                    click: function() {
                        r.removeClass("current-item"), jQuery(this).addClass("current-item"), e.onShowImage(jQuery(this).data("tab"))
                    }
                })
        ...
        onShowImage(e) {
            this.elements.$image_items.removeClass("current-item"), this.elements.$gallery_items.find("#" + e).addClass("current-item")
        }
    }
